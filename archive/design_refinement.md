...
- **Natural-as-location** is a spatial-concentration question that may need a finer grain than the Level III ecoregion.
  > **Amended 2026-08-04 (W6).** The open grain question above is now **answered: yes, finer — res-5 H3 hexes** (~62,494 acres). The branch target also changed, from *burned-area concentration* to **ignition likelihood** ("where within a region are fires most likely to start?"). The lever argument at the top of this document is what drove it: because mitigation works are *sited*, ignition likelihood is a siting question regardless of cause, which makes it the more actionable target and removes the reason to restrict this branch to Natural-caused fire. The target switch also changes the geometry — ignition counts use **raw FPA-FOD points**, not the MTBS perimeters W5 built for acres. See `CLAUDE.md` → "Grain and targets (W5/W6)" and collaboration log Entries 6.2–6.3.
