...
One result recurs across the project and is worth stating separately, because building the storyboard is what made it visible as a single thing. Every failure this project has recorded is a **grain mismatch**: W4's climate covariates carried a real per-region signal that averaged to zero when pooled across 105 regions; the hex-grain covariates identify dry *places* rather than dry *years*; the ecoregion turned out too coarse to site anything, which the hex fixed; and burned-area size is unforecastable over a season, which the same logic suggests may need the day. Three of those were diagnosed separately before the fourth made the pattern obvious. The practical form of it is that the model's unit has to match the unit the phenomenon actually varies on — and where it does, cheap history beats sourced covariates every time.
...
- **Done** — Findings and the storyboard: nineteen assertion headlines in delivery order, each with a figure, and the recommendation reduced to one sentence the deck opens and closes on.
- **In progress** — The final deck: laying the nineteen figures into slides, and the W6 practice recording against the five-minute subset.
...
> **Stop targeting how big it gets. Target where it starts.** Rank causes by region, site the work by
> ignition, and fix the record where it says neither — the order is trustworthy, the acre level much
> less so.

Reworded this week to lead with the change being asked for rather than with the three products that
deliver it. The previous version — *"target prevention where the region's own history says the causes
are, site mitigation where it says fires start, and fix the record where it can't say either"* — is
accurate and describes the deliverables, but a planner reading only the first line should learn what
to do **differently**, and that is the shift away from forecasting fire size. Beat 19 closes on the
same sentence, so the deck's first and last statements match.

**Deliberately a targeting claim, not an efficacy one.** Nothing in this project measures what a
treatment achieves; the ranking says where fire is most likely to arrive, which is a necessary
condition for sited work to pay off and not a sufficient one.
...
| 19 | Target causes by region. Site the work by ignition. Fix the record where it says neither. | `img/w6_recommendation.png` — three rows, one per Tier-1 class, each with its share of acres, its instruction and its grain: Natural **58.9%**, site by ignition, hex-season; Human **22.7%**, rank causes by the acres they drive, ecoregion-season; Unknown **18.5%**, fix the record underneath, ecoregion-season. Ordered by share rather than by the headline's clause order, so the top row is where the acres are. The Tier-1 split from beat 3 returns with a deliverable on each class, which makes the three-part recommendation structural rather than three unrelated tips. **The slide closes on the recommendation itself — "Stop targeting how big it gets. Target where it starts" — which recovers the instruction that came off beat 15's headline**, with its boundary on the line beneath: rank on all three, the order is trustworthy and the acre level much less so. A statement rather than a chart; see below. |
...
That keeps the spine intact — fire is seasonal, cause is regional, the mix is forecastable, starts
are predictable at hex grain, the acres sit in almost no cells, the grain diagnosis, the gate, the
binary rule, the recommendation. What it drops is every beat whose job is to *defend* a result rather
than state one: the k-sweep (4), the losing learned rung (6), the shuffled controls (8), the
four-season human profile (9), the two-branch tail split (11), and the whole covariate arc (12–14).
Those are the beats most likely to draw questions, which is exactly why they stay in the final and
come out of the rehearsal.

**Two beats drop for a different reason.** Beat 5 (the Human sub-cause floor) is a result rather than
a defense, but the five-minute cut carries only one level of the hierarchy, and beat 3's tiles teach
the encoding that beat 5 reuses — so the deeper level goes and the shallower one stays. Beat 18
(Unknown) is a third of the recommendation and survives as a row on beat 19, but at five minutes a
track that appears once cannot carry its own slide.

**What the cut costs, stated plainly.** Without 11–14 the five-minute version asserts that acres
concentrate (10) and then jumps to the grain diagnosis (15) without ever showing the forecast failing
on the tail or the covariates failing to fix it. Beat 15 has to carry that on the strength of the
speaker's word. That is acceptable in a rehearsal and would not be in the final, which is the reason
the full nineteen exist.

### Headline-only test

Read as a sequence with the slides removed, the nineteen assertions carry the argument: fire is
seasonal and cause is regional (1–2), what to target and why history suffices (3–6), where to site
and why the skill is real (7–9), where the acres actually sit and where the branches diverge (10–11),
what fuel data can and cannot buy (12–14), the grain diagnosis that bounds the null (15), the reframe
that turns it into a recommendation (16–17), where the record itself is the weak link (18), and the
three-part recommendation (19).

**The test is self-administered so far, which is its weakness.** The assignment runs it with a
podmate reading only the headlines, and that is the version that finds gaps — an author reading their
own headlines supplies from memory the connective tissue a listener does not have. Everything below is
what the test surfaced from the inside; the pod-feedback list names what I most need an outside
reader to check.

**The descent, and what carries a listener through it.** Beats 10 to 15 are six consecutive
statements of what does not work, which is the longest stretch in the deck without a positive result.
Three things keep it from reading as a list of disappointments. The run opens on a fact about fire
rather than a forecast failure — almost all the acres are in almost none of the cells — so the stakes
are set before anything fails. Beat 12's headline frames the covariate arc as a repair attempt, so
12–14 are *what we did about it* rather than three more losses. And beat 15 ends the descent by
naming the failure's shape instead of restating it, which is what lets beat 16 move upstream.
Whether that is enough is the second question in the pod-feedback list below.
...
composition to ignition counts, and the unit from shares to counts. Three changes at one boundary,
and a listener has no reason to expect any of them. Beat 7's headline carries the grain change —
*"but not at ecoregion scale"* names what is being left behind rather than only what is arriving.
The target change is not something a headline can hold, so it is a spoken line in `talk_notes.md`:
this beat predicts **where fires start**, not how much burns, and the two are not the same model.
Left unsaid, the capture curve reads as "32% of the *burn* under 6% of the ground," which is a
stronger claim than the evidence supports.

**The second seam, at 11→12.** The same defect in a different form. Beat 11 ends on a failure and
beat 12 opens on drought and vegetation indices, which invites reading the covariate arc as a change
of subject when it is the *response* to the failure just shown. Nobody sourced climate and NDVI out
of curiosity — they were sourced to fix exactly what beat 11 exposes. Beat 12's headline says so
outright (*"we tried to fix it with drought and fuel, but..."*), which makes 12–14 a repair attempt
with a verdict rather than three unexplained slides of covariate results. It also changes what the
null *means*: a covariate that fails after being introduced as a fix is evidence about the problem,
where the same covariate failing in a vacuum is just a dead end.

**The two slides that show no data, and why.** Beats 15 and 19 are the only figures in the deck
that plot nothing, and both earn it the same way: what they need to show is not a measurement.
Beat 15 draws the *scope* of a null — the grain drop that worked beside the one that was never
tested — because every number it could plot had already been plotted three beats earlier, and
re-plotting it would invite the audience to re-audit numbers at the moment the talk needs them
accepting a conclusion. Beat 19 draws the *structure* of the recommendation, because its headline is
three instructions pointing at three products at two grains: any single chart has to pick one and
demote the other two, and each candidate — the siting map, the triage list, the Tier-1 tiles — is
already a slide the audience has seen. What had never been shown is all three together, which is the
one thing that makes the recommendation read as architecture rather than as three unrelated tips.

The cost is that a deck partly judged on visuals now ends with a no-data slide. Three beats separate
them, and beats 16–18 are all figures, so the run does not read as a deck that ran out of material.

**The buried lead, and where it moved.** The first storyboard was twelve beats in chronological
order — build the hex grid, fetch the covariates, run the ablations, discover the tail failure, then
notice the ignition gate — which buried the lead at **position 11 of 12**. It had a second and larger
problem: it opened on the hex natural surface, which is *one of three tracks* in a two-tier
architecture, and presented it as though it were the whole product. RQ2's actual deliverable, the
region-season cause profile, appeared nowhere. The rewrite restored the hierarchy — Tier 1 establishes
**what**, hex grain answers **where**, the recommendation joins them — and the ignition gate now
arrives as **beat 16 of 19**, where it opens the recovery rather than closing the deck.

Two further moves followed. Neither was a buried lead; both were beats sitting where they could not
do their work.

| what moved | from → to | why |
| --- | --- | --- |
| the Unknown branch | 7 → 18 | It is a recommendation about the record, not a finding, and at beat 7 it interrupted the Human thread to deliver something a planner could not act on for another eleven beats. Against beat 19 it becomes the third of three Tier-1 products, all on screen together. The cost: Tier 1 introduces a class at beat 3 that nothing exercises until 18, so beat 19 reintroduces it in a sentence. |
| the concentration curve and the tail failure | 11, 10 → 10, 11 | The stakes have to precede the failure. Naming that almost all the acres sit in almost none of the cells *before* showing the forecast break there makes beat 11 a payoff instead of one more disappointment. |

The through-line in all three is the same: chronology is a fact about the project, and the audience
does not care in what order things were tried.

**Why the concession and the reframe are adjacent.** Beat 15 and beat 16 are the deck's most fragile
join — a listener has no reason to expect that "size is not forecastable before the season" leads to
"every megafire was an ignition first," and read carelessly they are a failure followed by an
unrelated consolation. Beat 15's headline is a *diagnosis* rather than a bare null for exactly this
reason: naming the failure as a grain mismatch points somewhere, and where it points is upstream, at
the ignition. Beat 11 carries a second load in the same service — showing that only lightning fire
has a runaway tail is what licenses beat 19 to ship two *different* products, so it cannot be
compressed away.

**Why the covariate arc is three beats and not one.** Reported as a single null it is a
disappointment. Split, it is a contrast: the same fuel data that says nothing about *where a fire
starts* (12) does add real signal about *how much burns* (13, +0.049 at 26.6 SD above a shuffled
control), and then lands on 1–20 acre fires while the tail goes 855x to 868x (14). That contrast is
what explains why the recommendation sites against starts rather than against predicted burn size.

**What still needs pod feedback.** Two questions, in order of how much they would change.

First, whether the deck reads as one argument or as two stapled together — the Level III *what* and
the hex-grain *where*. Beat 7's headline and a spoken line in `talk_notes.md` carry the join;
whether they are enough is exactly what an outside listener can tell me and I cannot.

Second, whether beats 10–15 read as a descent. Six consecutive negative results is the longest
stretch in the deck without a positive one, and each is load-bearing: without 11 the two branches
are not distinguished, and without 12–14 beat 15's diagnosis is unsupported. If a podmate loses
interest anywhere in that run, the ordering is not carrying it and compression is the answer.

The compression candidate if it runs long is still beat 4 — it tunes a parameter of the beat-3
baseline rather than adding a claim, and its finding is that the parameter barely matters. Beat 6
(the learned rung losing) reads as the same "the simple thing won" note but earns its place: it is
the only beat that concedes a model was tried and lost, which is what keeps the nulls credible.
...
- `src/w6_visuals.py`, `notebook/15_w6_visuals.ipynb` — **the deck's figures, one per storyboard beat,
  nineteen of them.** Every number a figure prints is either computed from a cached panel or
  transcribed from the notebook that published it, with a module-level source constant naming the
  cell — so a figure cannot drift from the report that quotes it. Four decisions govern the set:
  - **Reuse an axis rather than teach a new one.** Beat 11's "cells ordered by acres burned, least to
    most" appears three times (10, 11, 14), so beats 11 and 14 spend their attention on the finding
    rather than on the frame. The Tier-1 tiles (3) and the Human tiles (5) share `_draw_score_tiles`
    for the same reason: one question at two levels should not be two encodings.
  - **Draw the null on a scale where a real effect would have shown.** The ignition ladder (12) runs
    from zero, so that four flat rungs read as flat rather than as noise magnified by a cropped axis;
    the acres ladder (13) uses the same zero-based scale for the opposite reason, so its one real gain
    is not inflated by the axis into something beat 14 then has to deflate.
  - **Two slides plot no data**, because what they show is not a measurement: beat 15's grain parallel
    and beat 19's three products. Both are argued above.
  - **Populations are named wherever two figures could be mistaken for one comparison.** The ignition
    and acres ladders look deliberately alike and are scored on different targets, baselines and cell
    counts (29,293 hex-seasons against 7,799 burning cells); the storyboard says "comparable in shape,
    not cell for cell" so the resemblance cannot be misread.

  The two figures held for Q&A rather than the deck are the NDVI variance decomposition (between-place
  spread 2.8x the within-hex spread, which is the *mechanism* behind beat 12's flat line) and the
  region volatility map. Both answer questions well and neither survives the glance test as a slide.
...
- **Record the five-minute practice talk and post it with the self-assessment.** The cut is specified
  above — beats 1, 2, 3, 7, 10, 15, 16, 17, 19 — and every one of those beats has its figure, so what
  is missing is slides to record against and the recording itself.
- **Run the headline-only test with a podmate reading, and complete the structure peer-review.** The
  test in this report is self-administered, which is the weakest form of it: the author cannot hear
  the gaps the headlines leave. Two podmate drafts also need a buried lead found and relocated.
- **Build the final deck against this storyboard, one slide per assertion.** All nineteen beats have
  a figure, rendered from `notebook/15_w6_visuals.ipynb`, so what remains is layout rather than
  construction. Two constraints to respect while laying them in: beat 4 is the compression candidate
  if it runs long, and beats 10 and 11 are a matched pair — 10 sets the stakes, 11 is the finding that
  lands on them — so neither survives alone.
- **Test whether same-day conditions — wind, timing, suppression availability — predict which igniting
  natural hexes escape.** Beat 15 states this as a grain hypothesis rather than a bare null: the
  season is too coarse a unit to size a fire, exactly as the ecoregion was too coarse to site one.
  That makes it the project's clearest next experiment and also the most expensive — a different data
  requirement, not a tuning exercise on the current panel.
