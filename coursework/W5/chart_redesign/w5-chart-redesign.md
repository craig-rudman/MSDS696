...
| 6 | **Labelled three points directly** (~410, ~355, ~578) instead of none. | The original made the viewer decode every value off an inverted axis. Naming the start, the trough, and the endpoint puts the numbers the argument rests on directly in the reader's eye. |
| 7 | **Removed the gridlines and sized the type for projection** (1920×1080, 19–30pt). | Assumes the chart is shown on a screen rather than read on paper. With the three key values labelled, ruled lines competed with the series for attention at distance without adding information. |

**Data note:** the underlying series was never published, so values are recovered from the original PNG by pixel measurement rather than estimated by eye. Its axes were calibrated from their own tick-label centres — 300 → row 143.0 and 700 → row 717.0 vertically, 2012 → column 232.5 and 2024 → column 1304.0 horizontally — and the 2018 gridline predicted by that fit falls within 0.25 px of its actual position, so the mapping is sound. Values remain approximate and are labelled as such throughout.
